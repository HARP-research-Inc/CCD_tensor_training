...
class Category(object):
	...
	model_cache: ModelBank = ModelBank("models/discobert")
...
class Box(Category):
	...
	def check_packet_status(self):
		...
		for pair in self.inward_requirements:
			...
			status = status or interpret_requirement(0 if pair[0] not in self.grammar_data_cache else self.grammar_data_cache[pair[0]], pair[1])
			
			#print(0 if pair[0] not in self.grammar_data_cache else self.grammar_data_cache[pair[0]], pair[1])
			#print(pair, status)
			#if status == False:
			#	return status
		return status 
...
class Circuit(Category):
	...
	def set_levels(self, levels: list[list[Box]]):
		"""
	...
	def concactenate(self, other, conjunction: Box = None):
		...
		self.levels = other.get_levels()
	
	def forward(self):
		currentLevel = len(self.levels) - 1
		queue: list[Box] = self.levels[currentLevel]
		
		for i in range(len(self.levels)):
			print(i, [b.label for b in self.levels[i]])
	...
	"""def forward(self):
		stack: list[Box] = self.sources.copy()
		explored = set()
		last_output = None
		
		while len(stack) > 0:
			v = stack.pop()  # Pop from end for DFS (LIFO)
			print("\n\n********Current node:********:", v.get_label())
			print("Depth-first traversal stack: ", [s.get_label() for s in stack])
			
			if v.check_packet_status():
				last_output = v.forward()
				
				# Add children to stack in reverse order for reverse DFS
				children = []
				for wire in v.out_wires:
					print("Wire", wire.get_label())
					if wire.get_sink() not in explored:
						print("Sink", wire.get_sink_label())
						explored.add(wire.get_sink())
						children.append(wire.get_sink())
				
				# Add children in reverse order to achieve reverse DFS
